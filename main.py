...
# ============================================================
# NIFTY PRO QUANT TERMINAL v18.3
# GOOGLE LIVE NEWS RSS FIX + FULL TRADE JOURNAL + LOCAL MENTOR
# ============================================================
...
# ============================================================
# NEW ROCK-SOLID GOOGLE NEWS RSS INTEGRATION (NO MORE RUSSIAN!)
# ============================================================
def fetch_live_news():
    try:
...
def main(page: ft.Page):
    ...
    state = {
        ...
        "sl_val": 0.0, "target_val": 0.0, "order_type": "CE", "order_lots": 1, "live_ce_prem": 0.0, "live_pe_prem": 0.0, "atm_strike": 0,
        "news_items": [], "notes": []
    }
    ...
    def send_ai_message(e):
        ...
        def worker():
            ...
            time.sleep(0.5) 
            chat_list.controls[-1] = ft.Text(f"🤖 Mentor:\n{ans}", size=10, color=ft.colors.CYAN_200)
            chat_list.update()
            ...
            note_ui = ft.Container(
                ...
                content=ft.Row([
                    ft.Column([
                        ...
                        ft.Text(note["text"], size=10, color=ft.colors.WHITE)
    ...
    left_panel = ft.Container(
        ...
        content=ft.Column([
            ft.Row([ft.Icon(ft.icons.SECURITY, color=ft.colors.GREEN_400, size=14), ft.Text("INSTITUTIONAL FLOW", weight=ft.FontWeight.BOLD, color=ft.colors.GREEN_400, size=11)]),
            ft.Container(bgcolor="#020409", padding=6, border_radius=6, content=ft.Column([ft.Row([ft.Text("BIG MONEY:", size=8, color=ft.colors.WHITE54), big_money_status]), ft.Row([ft.Text("VOLUME:", size=8, color=ft.colors.WHITE54), volume_spike_status]), ft.Row([ft.Text("OI DATA:", size=8, color=ft.colors.WHITE54), oi_change_status])], spacing=2)),
            ...
            ft.Divider(color=ft.colors.WHITE24, height=6),
            ft.Row([ft.Icon(ft.icons.SUPPORT_AGENT, color=ft.colors.BLUE_400, size=14), ft.Text("SMART LOCAL MENTOR", weight=ft.FontWeight.BOLD, color=ft.colors.BLUE_400, size=11)]),
            ft.Container(content=chat_list, height=130, bgcolor="#020409", padding=6, border_radius=6),
            ...
            ft.Divider(color=ft.colors.WHITE24, height=6),
            ft.Row([ft.Icon(ft.icons.EDIT_NOTE, color=ft.colors.YELLOW_400, size=14), ft.Text("TRADER'S NOTEPAD", weight=ft.FontWeight.BOLD, color=ft.colors.YELLOW_400, size=11)]),
            ft.Container(content=notes_list, expand=True, bgcolor="#020409", padding=4, border_radius=6),
            ft.Row([notepad_input, ft.ElevatedButton("Add", bgcolor=ft.colors.YELLOW_800, color="white", height=35, on_click=add_new_note)], spacing=2)
        ], spacing=4)
    )
    ...
    right_panel = ft.Container(
        width=300, expand=True, bgcolor="#0A1128", border_radius=12, padding=12, border=ft.border.all(1, "#1C2A4A"),
        content=ft.Column([
            ft.Row([ft.Icon(ft.icons.GRID_VIEW, color=ft.colors.ORANGE_400, size=14), ft.Text("NIFTY SECTORS WATCH", weight=ft.FontWeight.BOLD, color=ft.colors.ORANGE_400, size=11)]),
            ...
            ft.Row([ft.Icon(ft.icons.SATELLITE_ALT, color=ft.colors.PURPLE_400, size=14), ft.Text("MACRO & GLOBAL RADAR", weight=ft.FontWeight.BOLD, color=ft.colors.PURPLE_400, size=11)]),
    ...
    ledger_dialog = ft.AlertDialog(title=ft.Text("PAPER TRADE HISTORY (LEDGER)", color=ft.colors.CYAN_400, weight=ft.FontWeight.BOLD), content=ft.Container(width=700, height=400, bgcolor=ft.colors.BLACK87, padding=10), actions=[])
    ...
    def open_ledger(e):
        ...
        except Exception as ex: ledger_list.controls.append(ft.Text(f"Error loading ledger: {ex}", color=ft.colors.RED_400))
    ...
    order_pad = ft.Column([
        ft.Row([order_header_txt, ledger_btn], alignment=ft.MainAxisAlignment.SPACE_BETWEEN), ft.Divider(height=2, color=ft.colors.WHITE24),
        ft.Row([ce_order_btn, pe_order_btn], alignment=ft.MainAxisAlignment.CENTER),
    ...
    portfolio_pad = ft.Column([
        ft.Row([ft.Text("LIVE PORTFOLIO", weight=ft.FontWeight.BOLD, color=ft.colors.CYAN_400), ft.Row([ledger_btn, capital_text])], alignment=ft.MainAxisAlignment.SPACE_BETWEEN), ft.Divider(height=2, color=ft.colors.WHITE24),
        ft.Row([
    ...
    def fetch_all_data():
        ...
        def task():
            try:
                ...
                if price > vwap_val and candle_bull and e4_bias == "BULLISH": signal, signal_color, entry, target, stop = "BULLISH SETUP (CALL)", ft.colors.GREEN_400, price, price + max(atr_val * 0.35, 1), price - max(atr_val * 1.5, 1)
                elif price < vwap_val and not candle_bull and e4_bias == "BEARISH": signal, signal_color, entry, target, stop = "BEARISH SETUP (PUT)", ft.colors.RED_400, price, price - max(atr_val * 0.35, 1), price + max(atr_val * 1.5, 1)
                else: signal, signal_color, entry, target, stop = "NO TRADE ZONE / CHOPPY", ft.colors.YELLOW_400, price, price, price
    ...
    controls_row = ft.Row([ft.ElevatedButton("LIVE SCAN", icon=ft.icons.RADAR, bgcolor=ft.colors.BLUE_700, color=ft.colors.WHITE, on_click=lambda e: fetch_all_data()), ft.ElevatedButton("ULTIMATE SCALP", icon=ft.icons.STAR, bgcolor=ft.colors.PURPLE_700, color=ft.colors.WHITE, on_click=run_backtest), auto_scan_switch], alignment=ft.MainAxisAlignment.SPACE_EVENLY)
    ...
    def clock_worker():
        while state["running"]:
            time_text.value = datetime.now().strftime("%d %b %Y | %H:%M:%S")
            try: time_text.update()
            except: pass
            time.sleep(1)

    dev_dialog = ft.AlertDialog(title=ft.Text("WATCHDOG (Glitch & Lag History)", color=ft.colors.ORANGE_400, weight=ft.FontWeight.BOLD), content=ft.Container(dev_list, width=600, height=350, bgcolor=ft.colors.BLACK87, padding=10), actions=[ft.ElevatedButton("Clear Logs", on_click=lambda e: (state["glitch_logs"].clear(), open_dev_menu(e)))])
    ...
    center_panel = ft.Container(
        expand=True, content=ft.Column([
            ...
            ft.Row([price_text, live_status], alignment=ft.MainAxisAlignment.SPACE_BETWEEN), chart_container, data_row, 
            ft.Row([box_news, box_tech], alignment=ft.MainAxisAlignment.SPACE_BETWEEN), ft.Row([box_live, box_engine4], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
